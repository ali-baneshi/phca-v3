...
این سند برای کسی نوشته شده است که هیچ پیش‌زمینه‌ای از پروژه ندارد، اما می‌خواهد سریع و عمیق بفهمد PHCA Cognitive Observatory چیست، چرا ساخته شده، روی چه پایه‌های علمی و مهندسی ایستاده، در فاز ۷ از یک مسیر ۲۰ فازی چه جایگاهی دارد، چه محدودیت‌هایی دارد، برای چه کاربردهایی مناسب است، و مسیر توسعه‌ی درست آن باید چگونه باشد.
...
این سیستم در فاز ۷ قرار دارد، یعنی هنوز محصول نهایی نیست. فاز ۷ را باید «فاز مشاهده‌پذیری شناختی و شفاف‌سازی رفتار» دانست، نه فاز بلوغ کامل محصول. از ۲۰ فاز کل مسیر، فاز ۷ نقطه‌ای است که سیستم دیگر فقط اجرا نمی‌شود، بلکه باید بتواند به انسان توضیح دهد که در هر لحظه چه دیده، چه پیش‌بینی کرده، چرا یک عمل را انتخاب کرده، چه خطاهایی داشته، چه منابعی مصرف کرده، و آیا replay همان چیزی را نشان می‌دهد که live نشان داده است یا نه.
...
این پروژه در فاز ۷ از ۲۰ فاز قرار دارد. این نکته مهم است، چون نباید انتظارات فاز ۱۵ یا ۲۰ را به فاز ۷ تحمیل کرد.
...
از فاز ۸ تا ۲۰، انتظار می‌رود سیستم به سمت بلوغ بیشتر برود:

- مشاهده‌پذیری دقیق‌تر و کم‌هزینه‌تر.
- schema versioning رسمی.
- تست‌های رفتاری کامل‌تر.
- گزارش‌های تحلیلی عمیق‌تر.
- داشبورد قابل توسعه‌تر.
- پشتیبانی از sessionهای بزرگ‌تر.
- مقایسه‌ی چند session.
- ابزارهای تشخیص anomaly.
- توضیح‌پذیری بهتر decisionها.
- قابلیت استفاده در پژوهش، آموزش، benchmark و audit.
...
این سیستم در فاز ۷ است، بنابراین محدودیت طبیعی دارد.

### محدودیت schema

schema هنوز version رسمی ندارد. اگر فیلدی اضافه یا حذف شود، replay قدیمی ممکن است silent degrade شود.

راه درست:

- افزودن schema_version.
- تست roundtrip.
- مستند کردن live-only fields.

### محدودیت performance

جلسات ۱۵۰۰ یا ۳۰۰۰ cycle می‌توانند UI را سنگین کنند.

ریسک‌ها:

- rebuildهای غیرضروری.
- paint-time computation.
- sort/argsort در هر repaint.
- QPixmap cache نامعتبر.

راه درست:

- cache با کلید درست.
- rebuild فقط هنگام seek/jump.
- append فقط هنگام sequential live tick.
- محدود کردن history window.

### محدودیت report parity

گزارش آفلاین هنوز تمام قابلیت‌های dashboard را پوشش نمی‌دهد.

راه درست:

- helperهای pure در `cognitive_panels.py` منبع حقیقت باشند.
- dashboard و report هر دو از helper مشترک استفاده کنند.
- report فقط از JSONL استفاده کند.

### محدودیت legacy render

مسیر matplotlib/static replay هنوز وجود دارد ولی با PyQt dashboard هم‌سطح نیست.

راه درست:

- یا آن را legacy و محدود اعلام کنیم.
- یا منطق مهم replay را shared کنیم.
- یا در آینده حذف/ایزوله شود.
...
PHCA Cognitive Observatory در فاز ۷ یک ابزار تزئینی نیست؛ بخشی از معماری اعتماد سیستم است. این لایه تعیین می‌کند آیا انسان می‌تواند رفتار عامل را بفهمد، replay کند، گزارش بگیرد و خطاهای شناختی یا منابعی را پیدا کند یا نه.
