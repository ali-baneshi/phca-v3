...
| **A4** Prediction + Spatial Heuristics as Hybrid Cognitive Map | G′ predict every cycle, environment-scoped (D-158); GridWorld geometry as inductive bias | A4 test on all envs | **GridWorld: pure BFS/Manhattan geometry (default since D-156)** — environment-scoped A4 (D-158); blended G′ scorer opt-in via `--enable-blended-scorer` with agreement-based gating (D-159) as safety net. Prediction-primary for continuous-control (MuJoCo MPC). See DECISIONS.md D-156/D-158/D-159. |
