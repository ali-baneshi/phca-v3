...
| **A4** Prediction + Spatial Heuristics as Hybrid Cognitive Map | G′ predict every cycle, environment-scoped (D-158); GridWorld geometry as inductive bias | A4 test on all envs | **GridWorld: pure BFS/Manhattan geometry (default since D-156)** — environment-scoped A4 (D-158); blended G′ scorer opt-in via `--enable-blended-scorer` with agreement-based gating (D-159) as safety net. Partial-observability viewport support (D-160): `partial_obs_radius` parameter, `observed_grid` property, `_observed_cells` frontier tracking, cycle plans through only known walls. Frontier-based exploration when goal unseen (D-160 extension): `_find_frontier_cell()` directs agent toward unexplored areas. RBTA entropy-floor fix under partial obs (D-160 extension): divide (not multiply) `entropy_floor` by scale, apply scaling after `gprime_stress_bounds()` overwrite. See DECISIONS.md D-156/D-158/D-159/D-160. |
